...
template<typename T>
inline T get_norm(T* v, int f, int p) {
  T norm = 0;
  for (int z = 0; z < f; z++)
	norm += pow(v[z], p);
...
template<typename T>
inline void normalize(T* v, int f, int p) {
  T norm = get_norm(v, f, p);
  for (int z = 0; z < f; z++)
	v[z] /= norm;
}

template<typename T, typename Random, typename Distance, typename Node>
inline void two_means(const vector<Node*>& nodes, int f, Random& random, T* iv, T* jv) {
  /*
...
struct L1
{
	...
	static inline void create_split(const vector<Node<S, T>*>& nodes, int f, Random& random, Node<S, T>* n)
	{
		...
		two_means<T, Random, L1, Node<S, T> >(nodes, f, random, &best_iv[0], &best_jv[0]);
...
struct L2 {
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, Random& random, Node<S, T>* n) {
	vector<T> best_iv(f, 0), best_jv(f, 0);
	two_means<T, Random, L2, Node<S, T> >(nodes, f, random, &best_iv[0], &best_jv[0]);
...
struct L3
{
	...
	{
		vector<T> best_iv(f, 0), best_jv(f, 0);
		two_means<T, Random, L3, Node<S, T> >(nodes, f, random, &best_iv[0], &best_jv[0]);
...
public:
  ...
  bool load(const char* filename) {
	_fd = _open(filename, O_RDONLY, (mode_t)0400);
	if (_fd == -1)
	  return false;
  ...
  void _get_all_nns(const T* v, size_t n, size_t search_k, vector<S>* result, vector<T>* distances) {
	...
	while (nns.size() < search_k && !q.empty()) {
	  ...
	  if (nd->n_descendants == 1) {
		...
		q.push(make_pair(MIN(d, +margin), nd->children[1]));
		q.push(make_pair(MAX(d, -margin), nd->children[0]));
	  }
	}
